...
for command in git-comb git
	...
	complete -c $command -n __git_comb_using_command -l exclude-dirty -d 'Exclude repositories with uncommitted changes'
	complete -c $command -n __git_comb_using_command -l exclude-unpushed -d 'Exclude commits that exist on no remote'
	complete -c $command -n __git_comb_using_command -l exclude-ahead -d 'Exclude branches ahead of their upstream'
	complete -c $command -n __git_comb_using_command -l exclude-behind -d 'Exclude branches behind their upstream'
	complete -c $command -n __git_comb_using_command -l exclude-stashed -d 'Exclude repositories with stashes'
	complete -c $command -n __git_comb_using_command -l exclude-empty -d 'Exclude empty repositories'
	complete -c $command -n __git_comb_using_command -l exclude-local -d 'Exclude repositories without remotes'
	complete -c $command -n __git_comb_using_command -l exclude-offline -d 'Exclude remotes unreachable during --fetch'
	...
	complete -c $command -n __git_comb_using_command -l max-depth -r -a '0 1 2 3 4 5' -d 'Limit directory descent below each root'
